...
function complete_unit(){
  ...
  if(isset($coursetaken) && $coursetaken){
    ...
    $activity_id=bp_course_record_activity(array(
      'action' => __('Student finished unit ','vibe').get_the_title($unit_id),
...
function reset_question_answer(){
  ...
  if(isset($ques_id) && $_POST['security'] && wp_verify_nonce($_POST['security'],'security'.$ques_id)){
    ...
    echo __('Answer Reset','vibe');
  }else
    echo __('Unable to Reset','vibe');
...
function reset_course_user(){
    ...
    if(isset($status) && $status){  // Necessary for continue course
      ...
      do_action('wplms_student_course_reset');
...
function complete_course_marks(){
    ...
    if(update_post_meta( $course_id,$user_id,$marks)){
      $message = __('You\'ve obtained ','vibe').$marks.__(' out of 100 in Course :','vibe').' <a href="'.get_permalink($course_id).'">'.get_the_title($course_id).'</a>';
    ...
    $activity_id=bp_course_record_activity(array(
      ...
      'content' => __('Student ','vibe').bp_core_get_userlink( $user_id ).__(' got ','vibe').$marks.__(' out of 100 in course ','vibe').get_the_title($course_id),
      'primary_link' => get_permalink($course_id),
      ...
      foreach($members as $member){
        ...
        if($user_id){
          if(update_post_meta($course_id,$user_id,0)){ // Move forward only if update is successful
           $course_duration = get_post_meta($course_id,'vibe_duraiton',true);
           $duration = time() + $course_duration*86400;
      ...
      the_unit($unit_id);  
              ...
              if($prev >=0){
                ...
                if(get_post_type($units[$prev]) == 'quiz')
                  echo '<a href="'.get_permalink($units[$prev]).'" class=" '.$unit_class.'">'.__('Back to Quiz','vibe').'</a>';
                else    
              ...
              if($next <= $max){
                ...
                if(get_post_type($units[$next]) == 'quiz')
                  echo '<a href="'.get_permalink($units[$next]).'" id="next_unit" class=" '.$unit_class.'">'.__('Proceed to Quiz','vibe').'</a>';
                else  
