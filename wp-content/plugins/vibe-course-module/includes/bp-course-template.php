...
function bp_course_get_students_undertaking($course_id=NULL, $page=0){
	global $wpdb,$post;
	if(!isset($course_id))
		...
		if(!$flag){
			...
			if(vibe_validate($auto_eval)){
			    ...
			    if(update_post_meta( $id,$user_id,$marks)){
			      $message = __('You\'ve obtained ','vibe').$marks.__(' out of 100 in Course :','vibe').' <a href="'.get_permalink($id).'">'.get_the_title($id).'</a>';
			    ...
			    bp_course_record_activity(array(
			      ...
			      'content' => __('Student ','vibe').bp_core_get_userlink( $user_id ).__(' got ','vibe').$marks.__(' out of 100 in course ','vibe').get_the_title($id),
			      'primary_link' => get_permalink($id),
