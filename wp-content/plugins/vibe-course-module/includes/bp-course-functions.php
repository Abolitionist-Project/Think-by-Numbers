...
function the_course_button($id=NULL){
  ...
  if(!is_user_logged_in() && vibe_validate($free_course)){
   ...
   if(isset($coursetaken) && $coursetaken && is_user_logged_in()){   // COURSE IS TAKEN & USER IS LOGGED IN
          ...
          $course_user=get_post_meta($course_id,$user_id,true); // Validates that a user has taken this course
            ...
            if((isset($course_user) && $course_user !='') || (isset($free_course) && $free_course && $free_course !='H' && is_user_logged_in())){ // COURSE PURCHASED SECONDARY VALIDATION
             ...
             echo  '</form>'; 
                  ...
                  echo '<a href="'.get_permalink($pid).'" class="'.((isset($id) && $id )?'':'course_button full ').'button">'.__('COURSE ENABLED','vibe').'</a><span>'.__('CONTACT ADMIN TO ENABLE','vibe').'</span>';   
            }
      }else{ 
      ...
      if(isset($pid) && $pid)
        echo '<a href="'.get_permalink($pid).'" class="'.((isset($id) && $id )?'':'course_button full ').'button">'.__('TAKE THIS COURSE','vibe').'</a>'; 
      else
  ...
  extract( $r, EXTR_SKIP );
   ...
   $return .= '<li><i class="icon-wallet-money"></i> <h5 class="credits">';
    ...
    if(isset($precourse) && $precourse!='')
        $return .= '<li><i class="icon-clipboard-1"></i> '.__('* REQUIRES','vibe').' <a href="'.get_permalink($precourse).'">'.get_the_title($precourse).'</a></li>'; 
           
   $return .= apply_filters('wplms_course_details_time','<li><i class="icon-clock"></i>'.get_the_course_time('course_id='.$course_id).'</li>'); 
...
if(!function_exists('tofriendlytime')){
  ...
  foreach ($measures as $label=>$amount) {
    if ($seconds >= $amount) {  
      ...
      $timelabel=apply_filters('wplms_time_labels',$label.($howMany > 1 ? "s" : ""));
...
function get_the_course_time($args){
  ...
  extract( $r, EXTR_SKIP );
      ...
      if(isset($d) && $d !='')
          ...
          return apply_filters('course_friendly_time',tofriendlytime(($d*86400)),($d*86400));
      }              
}
...
function bp_course_quiz_auto_submit($quiz_id,$user_id){
    ...
    if(vibe_validate($quiz_auto_evaluate)){ // Auto Evaluate Enabled
        $total_marks=0;
        $questions = vibe_sanitize(get_post_meta($quiz_id,'vibe_quiz_questions',false));
        if(count($questions)){
            $sum=$max_sum=0;
            ...
            bp_course_record_activity(array(
              ...
              'content' => __('Quiz ','vibe').get_the_title($quiz_id).__(' auto evaluated for student ','vibe').bp_core_get_userlink($user_id).__(' with marks ','vibe').$total_marks,
              'type' => 'evaluate_quiz',
              'primary_link' => get_permalink($quiz_id),
              'item_id' => $quiz_id,
              'secondary_item_id' => $user_id
