...
function wsl_render_login_form()
{
	...
	// fix for redirect_to parameter
	$is_login_page = in_array($GLOBALS['pagenow'], array('wp-login.php', 'wp-register.php'));
	
	// if you are on login page and no redirect_to parameter, redirect to homepage
	if ($is_login_page) 
	{
		$current_page_url = empty($_REQUEST['redirect_to']) ? home_url( '/' ) : esc_url($_REQUEST['redirect_to']);
	}
