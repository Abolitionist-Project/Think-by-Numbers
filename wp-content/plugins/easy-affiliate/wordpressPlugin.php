<?php
/*
Plugin Name: Easy Affiliate
Plugin URI: http://wordpress.org/extend/plugins/easy-affiliate-link-shortener
...
function easy_affiliate_installed() {
...
add_action('plugins_loaded', 'easy_affiliate_installed');
add_action('init', 'easy_affiliate_installed');

/* Runs when plugin is activated */
function easy_affiliate_activate() {
	...
	}else if( function_exists( 'paltipPluginInstalled' ) ) {
...
register_activation_hook(__FILE__,'easy_affiliate_activate');

/* Runs on plugin deactivation*/
register_deactivation_hook( __FILE__, 'easy_affiliate_remove' );
function easy_affiliate_remove() {
...
function easy_affiliate_admin_menu() {
	add_options_page(publicFunctions::$PLUGIN_NAME.' - settings', publicFunctions::$PLUGIN_NAME, 'manage_options','PalTip_options', 'easy_affiliate_html_page');
}

/**
 * Add Settings link to plugins - code from GD Star Ratings
 */
function add_easy_affiliate_settings_link($links, $file) {
	static $this_plugin;
...
if ( is_admin() ){
	/* Call the html code */
	add_action('admin_menu', 'easy_affiliate_admin_menu');
	add_filter('plugin_action_links', 'add_easy_affiliate_settings_link', 10, 2 );
}

function easy_affiliate_html_page(){
...
function easy_affiliate_update_links($content){
	$link_changer = new LinkChanger( publicFunctions::getPTID(), publicFunctions::getPTEmail() , publicFunctions::getPTUserName());
	return $link_changer->changeLinksInContent($content);
}
add_filter('content_save_pre','easy_affiliate_update_links');

function easy_affiliate_custom_dashboard_help() {
...
add_action('right_now_content_table_end', 'easy_affiliate_custom_dashboard_help');
?>