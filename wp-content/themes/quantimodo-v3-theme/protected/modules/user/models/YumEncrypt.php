	...
	public static function validate_password($password, $good_hash, $salt)
	{
		function changecase($str) {
			for ($i=0;$i<strlen($str);$i++) {
				if (ctype_lower($str[$i])) $str[$i]=strtoupper($str[$i]);
		...
		// second chance with changed case
		$enc_pwd2 = YumEncrypt::encrypt(changecase($password), $salt);
		if (YumEncrypt::slow_equals($enc_pwd, $good_hash)) return true;
		else return YumEncrypt::slow_equals($enc_pwd2, $good_hash);
	}	
